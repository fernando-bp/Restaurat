...
from sqlalchemy import JSON, Column, DateTime, DECIMAL, Integer, String, Text

from app.infrastructure.database.base import Base


class BoldQrPagoIndependienteORM(Base):
    __tablename__ = "bold_qr_pagos_independientes"

    id = Column(Integer, primary_key=True, autoincrement=True)
    orden_id = Column(Integer, nullable=True, index=True)
    ...
    webhook_evento = Column(JSON)
    ultimo_error = Column(String(500))
    confirmado_en_pos = Column(Integer, nullable=False, default=0)
    pago_pos_id = Column(Integer)
